...
export const SystemLogsPage: FunctionComponent = () => {
  ...
  return (
    <section className={styles["system-logs-page"]} data-component-id={componentId} data-component-uid={componentUid}>
      <header className={styles["timeline-header"]}>
        <p className={styles["timeline-kicker"]}>Observability</p>
        <h2>System Logs</h2>
        <p className={styles["timeline-subtitle"]}>Prompt submission/request/response files generated by the backend prompt workflow.</p>
      </header>

      <section className={styles["system-logs-controls"]}>
        <button
          type="button"
          onClick={() => {
            void loadLogs();
      ...
      <section className={styles["system-logs-layout"]}>
        <article className={styles["system-logs-list"]}>
          <h3>Files ({data?.logs.length ?? 0})</h3>
          {isLoading ? <p className={styles.subtle}>Loading files…</p> : null}
          {!isLoading && (data?.logs.length ?? 0) === 0 ? <p className={styles.subtle}>No log files found yet.</p> : null}
          <ul>
            {(data?.logs ?? []).map((entry) => (
              <li key={entry.id}>
                <button
        ...
        <article className={styles["system-logs-viewer"]}>
          <h3>{selectedLabel}</h3>
          <textarea
            readOnly
